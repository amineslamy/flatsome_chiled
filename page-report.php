...
<head>
    ...
    <link rel="stylesheet"
        href="<?php echo get_stylesheet_directory_uri(); ?>/assets/admin/css/jalali-datepicker.min.css">
    ...
    <style>
        ...
        * {
        ...
        .flat-card.card-green {
            border: 2px solid var(--sahab-green);
            ...
            <form method="GET" action=""
                ...
                <div class="flex flex-wrap items-center gap-3">
                    <div class="flex items-center gap-1">
                        ...
                        <input type="text" data-jdp name="start_date" value="<?php echo esc_attr($start_date); ?>"
                            placeholder="۱۴۰۵/۰۳/۰۱" autocomplete="off">
                    </div>
                    <div class="flex items-center gap-1">
                        <span class="text-xs font-semibold">تا تاریخ:</span>
                        <input type="text" data-jdp name="end_date" value="<?php echo esc_attr($end_date); ?>"
                            placeholder="۱۴۰۵/۰۴/۳۱" autocomplete="off">
                    </div>
                    ...
                    <a href="<?php echo strtok($_SERVER["REQUEST_URI"], '?'); ?>" class="flat-btn-ghost text-xs">حذف
                        فیلتر</a>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                    <?php if (!empty($departments)):
                        foreach ($departments as $m_id => $dept): ?>
                            <div class="flat-card">
                                <div class="text-sm font-black text-slate-800 dark:text-slate-100 mb-2">
                                    <?php echo esc_html($dept['name']); ?>
                                </div>
                                ...
                                <div class="flex justify-between text-xs mt-2">
                                    ...
                                    <span class="font-bold text-sky-600 dark:text-sky-400">
                                        <?php echo esc_html($dept['total_news']); ?> خبر
                                    </span>
                                </div>
                            </div>
                        <?php endforeach; else: ?>
                            ...
                            <?php if (!empty($analysts)):
                                ...
                                foreach ($analysts as $a_id => $analyst): ?>
                                    <tr>
                                        ...
                                        <td class="font-mono font-bold text-emerald-600 dark:text-emerald-400 text-center">
                                            ...
                                            <div class="flex flex-wrap gap-1.5">
                                                <?php foreach ($choices['subject'] as $sub_key => $sub_label): ?>
                                                    <?php if (isset($analyst['subjects'][$sub_key]) && $analyst['subjects'][$sub_key] > 0): ?>
                                                        <span
                                                            class="flat-badge border-slate-300 dark:border-slate-600 text-[10px] text-slate-500 bg-white dark:bg-slate-800">
                                                            <?php echo esc_html($sub_label); ?>: <span
                                                ...
                                                <?php foreach ($choices['evaluation'] as $eval_key => $eval_label): ?>
                                                    <?php if (isset($analyst['evaluations'][$eval_key]) && $analyst['evaluations'][$eval_key] > 0): ?>
                                                        <span class="flat-badge">
                                                            ...
                                                            <?php echo esc_html($eval_label); ?>: <span class="font-bold">
                                ...
                                <?php endforeach; else: ?>
                                <tr>
                                    <td colSpan="5" class="text-center py-6 text-slate-400 text-xs">هیچ کارشناسی در این بازه
                                        یافت نشد.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
